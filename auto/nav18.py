...
def generate_nav_dict(path, ignore=[]):
    ...
    for entry in entries:
        key = key_cleaner(entry.name)
        print(key)
        trunked_path = "./" + path[6:]
        if entry.name == "index.md":
            nav_dict['- '] = os.path.join(trunked_path,"index.md")
        elif entry.is_file():
            nav_dict[key] = os.path.join(trunked_path, entry.name)
